...
module.exports = {
	...
	getFolderFoldersById: function(req, res){
		...
		Folder.find({where: {parentFolder: id, library: libraryId}}).paginate({page: page, limit: 15}).exec(function(err, folders){
			if(err){
				res.serverError();
	...
	getFolderTracksById: function(req, res){
		...
		Track.find({where: {parentFolder: id}}).paginate({page: page, limit: 15}).exec(function(err, tracks){
			if(err){
				res.serverError();
