...
$(document).ready(function(){
	...
	$('#tab-ply').click(function(e){
		console.log("nte")
		request = new Request(124, "MOBILE", 19, 200, {});
		s.send(JSON.stringify(request));
	});
	...
	var check = false;
	if (!check) {
		$('#folder-selector').on("click", function(){
			request = new Request(124, "MOBILE", 16, 200, {parentFolder: "ROOT"});
			s.send(JSON.stringify(request));
		});
	} else {
		$('#folder-selector').addClass("disabled");
	}

	$('#play-all').click(function(e){
		request = new Request(124, "MOBILE", 18, 200, {});
		s.send(JSON.stringify(request));
	});
});
