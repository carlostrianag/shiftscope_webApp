...
class WebSocket(object):
    ...
    def feed(self, data, con_number):
        ...
        # We have handshaken
            ...
            JSONRequest = ''.join(recv)

            if JSONRequest[0] == '{':
                JSONObject = json.loads(JSONRequest)
                if JSONObject['type'] == 'new' or JSONObject['type'] == 'update':
                    TEAM_MEMBERS_LIST[con_number] = (JSONObject['member'])

                elif JSONObject['type'] == 'help':
                    TEAM_MEMBERS_LIST[JSONObject['to']]['helper'] = JSONObject['helper']
        ...
        while self.running:
            ...
            for ready in rList:
                if ready == self.socket:
                    ...
                    fileno = client.fileno()
                    if data:
                        self.connections[fileno].feed(data, fileno)
                    else:
                        logging.debug("Closing client %s" % ready)
                        self.connections[fileno].close()
                        del self.connections[fileno]
                        self.listeners.remove(ready)
                        if TEAM_MEMBERS_LIST[fileno] is not None:
                            del TEAM_MEMBERS_LIST[fileno]
                        print TEAM_MEMBERS_LIST.keys()

                    if LAST_CLIENT is not None:
                        self.connections[LAST_CLIENT].sendMessage(json.dumps({"type":"assignation", "id": LAST_CLIENT}))
                        LAST_CLIENT = None

                    for con in self.connections:
                    	try:
							self.connections[con].sendMessage(json.dumps({"type":"update", "content": TEAM_MEMBERS_LIST}))
                    	except Exception, e:
							file = open("Ulter_TS.log", "a")
							file.write(time.strftime("%c") + "\t" + e  +"\n")
							file.close()
...
if __name__ == "__main__":
    ...
    server = WebSocketServer("", 9876, WebSocket)
